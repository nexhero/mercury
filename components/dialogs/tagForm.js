...
export const TagFormProvider = ({children})=>{
  ...
  const [tagBuffer,setTagBuffer] = useState(null) // Store the document tag object
  const [opened,{open, close}] = useDisclosure(false);

  const [parent,setParent] = useState(''); // Store tag id
  ...
  const save = ()=>{
    ...
    console.log('Buffer object',tagBuffer)
    tagBuffer.save()
      .then(()=>handleClose())
