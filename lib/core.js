...
function useNoteFn(){
  ...
  const remove = (data)=>{
    return new Promise(async(resolve,reject)=>{
      try {
        if (data.type==='note') {
          await tableNote.del(data.value)
        }else{
...
function useRepositoryFn (){
  const {store,tableNote,setTableNote,publicKey} = useContext(PeerContext)

  const addReplicator = async(channel,name)=>{
    await tableNote.addChannel(channel,name)

  }
  const allChannels = ()=>{
    return new Promise(async(resolve,reject)=>{
      const arr = []
      for await (const channel of tableNote.getAllChannel()) {
        console.log('channel',channel)
        arr.push({
          id:channel.key,
          name:channel.value.name,
          publickey:channel.value.topic,
        })
      }
      resolve(arr)
    })
...
export const useRepository = useRepositoryFn
