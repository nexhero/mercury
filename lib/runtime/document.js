...
export const DocumentProvider = ({children})=>{
  ...
  const closeDocument = (id)=>{
    ...
    if (openedDocuments.size) {
      setActiveDoc(openedDocuments.entries().next().value[0])
    }else{
      setActiveDoc(null)
    }
  ...
  return html`
<${DocumentContext.Provider} value=${{setActiveDoc,activeDoc,documentList,tagList,listAllDocuments, newDocument,openedDocuments,openDocument,deleteDocument,deleteTag,closeDocument,closeAllDocuments}}>
  ${children}
<//>
`
}
